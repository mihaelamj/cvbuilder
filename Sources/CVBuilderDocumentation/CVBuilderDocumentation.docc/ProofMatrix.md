...
| R18 | Support structured results-oriented accomplishment fields and render them faithfully; never auto-inflate or invent metrics. | Keep; quantification sub-claim marked unproven | Adjacent | E32, E33 | `Project.accomplishments` carries action-plus-outcome statements rendered verbatim after the descriptions; no auto-superlativizing or fabricated numbers. Do not pressure users to add numbers; numeric-quantification effect is unsupported. |
...
- Skills counter-evidence (lower weight, not peer-reviewed): Fuller, Raman, Sigelman et al. (2024, Harvard Business School / Burning Glass Institute report) find that dropping degree requirements changed job postings far more than actual hires (about a 1-in-700 net benefit). It bounds the skills claim: structured, work-tied skills aid representation and matching legibility, not "listing skills gets you hired." Recorded for honesty; not weighted as proof.
- Advisory (not mandatory): an optional normalized skill identifier (an ESCO or O*NET code) per skill entry is permitted, because taxonomy normalization aids machine comparison across languages (E49, E50; ESCO and O*NET are reference standards, not findings). No located study evaluates CV rendering of such codes directly, so this stays advisory and off by default.
