...
| Add, revise, or retire a research rule (R01-R19) | <doc:ProofMatrix> first, then <doc:ConformanceMatrix> and the Swift rule mapping, then <doc:SourceMatrix> if a source changed | Each surviving rule still maps to at least one code location and one named test. |
