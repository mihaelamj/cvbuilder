...
- Issue #60: active clean SwiftPM consumer smoke test.
...
15. #60 - in progress: add a clean SwiftPM consumer smoke test.
...
- add a local or CI smoke test that creates a temporary Swift package: in
  progress
- import `CVBuilder` from that clean package: in progress
- on Linux, also import `CVBuilderTileDown`: in progress
- document the smoke test command if it is exposed locally: in progress

Acceptance:

- macOS proves `CVBuilder` consumption: in progress
- Linux proves `CVBuilder` and `CVBuilderTileDown` consumption: in progress
- smoke test stays Markdown and JSON only: in progress
